...
export default function RepositoryList() {
        ...
        <div className="space-y-4">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="flex items-center justify-between p-4 bg-slate-50 rounded-lg animate-pulse">
              ...
              <div className="w-16 h-8 bg-slate-300 rounded-lg"></div>
            </div>
          ))}
        </div>
      </div>
  ...
  if (error) {
    return (
      <div className="bg-white/80 backdrop-blur-sm rounded-lg border border-red-200 p-6">
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm border border-slate-200 rounded-lg p-6 h-fit">
      {/* Header */}
      <div className="mb-6">
      ...
      <div className="relative mb-6">
        <input
          ...
          className="w-full h-12 bg-white border border-slate-300 rounded-lg px-4 text-slate-900 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
        />
      ...
      <div className="space-y-3 mb-6">
        {filteredRepositories.length === 0 ? (
          ...
          filteredRepositories.map((repo) => (
            <div
              key={repo.id}
              className="group bg-white border border-slate-200 rounded-lg p-4 hover:bg-slate-50 hover:border-slate-300 transition-all duration-200 cursor-pointer"
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <div className="w-8 h-8 rounded-lg bg-orange-100 border border-orange-200 flex items-center justify-center flex-shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <h4 className="font-medium text-slate-900 group-hover:text-blue-600 transition-colors text-sm truncate">
                        {repo.name}
                      </h4>
                      {repo.private && (
                ...
                <button
                  ...
                  className="bg-slate-100 border border-slate-200 text-slate-700 hover:bg-slate-200 hover:border-slate-300 hover:text-slate-900 transition-all duration-200 font-medium text-sm h-8 px-3 rounded-md flex-shrink-0"
                >
                  Import
                </button>
              </div>
